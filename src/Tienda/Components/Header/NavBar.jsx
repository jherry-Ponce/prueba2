...
export const NavBar = () => {
  ...
  const handleChange = () => setScroling(!scroling);
  ...
  useEffect(()=>{
    categoryApi().then((cate)=>setCateg(cate))
  },[])
  let activeStyle = {
    background: '#198754', 
    color:'white',
  ...
  return (
    <>
      <Navbar
        collapseOnSelect
        expand={false}
        className="bg-dark position-sticky top-0 d-block py-0 z-index-100  "
      >
        <div className="">
              ...
              <div className=" row row-cols-4 position-relative">
                ...
                {categ.length>0?
                  categ.map((data, index) => (
                      <li key={index} className="navigation-link text-dark ">
                        <NavLink 
                         to={`/Category/${data.id}`}
